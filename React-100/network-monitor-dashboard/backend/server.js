...
app.get('/api/network/topology', async (req, res) => {
  try {
    ...
    // Quick ping test for online status
    ...
    // If still no gateway found, use common default
    if (!gatewayIP) {
    ...
    devicesExcludingGateway.forEach((device, index) => {
      ...
      if (gatewayIP) {
        ...
        if (device.status === 'online' && trafficLevel > 0.7) {
          connectionStyle.color.color = '#00ddff';
          ...
          connectionStyle.color.color = '#ffaa00';
